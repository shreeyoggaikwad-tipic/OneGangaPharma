...
//   queryKey: ["/api/medicines", { storeId }],   // 👈 include storeId in queryKey
//   queryFn: async ({ queryKey }) => {
//     const [_key, { storeId }] = queryKey as [string, { storeId: number }];

//     if (!storeId) {
//       return []; // if no storeId yet (user not loaded), return empty
//     }
...
//             <Card className="hover:shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer bg-gradient-to-br from-amber-50 to-yellow-50 border-amber-200 hover:border-amber-300">
//               <CardContent className="p-4 sm:p-6 text-center">
//                 <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-r from-amber-500 to-yellow-600 rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-lg">
...
//             <Card className="hover:shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer bg-gradient-to-br from-blue-50 to-cyan-50 border-blue-200 hover:border-blue-300">
//               <CardContent className="p-4 sm:p-6 text-center">
//                 <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-r from-blue-500 to-cyan-600 rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-lg">
//                   <Settings className="h-6 w-6 sm:h-8 sm:w-8 text-white" />
//                 </div>
//                 <p className="font-semibold text-gray-800 mb-1 text-sm sm:text-base">
//                   System Configuration
//                 </p>
//                 <p className="text-xs text-gray-600">Manage shelf life policies</p>
//               </CardContent>
//             </Card>
//           </Link>
...
export default function Dashboard() {
  ...
  // Get dashboard stats
  const { data: stats, isLoading: statsLoading } = useQuery<any>({
    queryKey: ["/api/admin/dashboard-stats", storeId],
    enabled: !!storeId,
  });
...
// const storeId = user?.storeId; // fallback to undefined if missing

const { data: orders = [], isLoading: ordersLoading } = useQuery<any[]>({
  queryKey: ["/api/admin/orders", storeId],
  queryFn: async () => {
    if (!storeId) return []; // avoid API call if no storeId
    const res = await fetch(`/api/admin/orders?storeId=${storeId}`);
    if (!res.ok) throw new Error("Failed to fetch orders");
    return res.json();
  },
  enabled: !!storeId,  // run only if storeId exists
  ...
  // Get low stock medicines
  const { data: lowStockMedicines = [] } = useQuery<any[]>({
    queryKey: ["/api/admin/low-stock", storeId],
    enabled: !!storeId,
  });

  // Get pending prescriptions
  const { data: pendingPrescriptions = [] } = useQuery<any[]>({
    queryKey: ["/api/admin/prescriptions", storeId],
    enabled: !!storeId,
  });
  ...
  const { data: stores = [], isLoading: storesLoading } = useQuery<any[]>({
    ...
    queryFn: async ({ queryKey }) => {
      const [_key, storeId] = queryKey as [string, number];
      if (!storeId) return [];
  ...
  // Get medicines data
  const { data: medicines = [], isLoading: medicinesLoading } = useQuery<any[]>({
    queryKey: ["/api/medicines", { storeId }],
    queryFn: async ({ queryKey }) => {
      const [_key, { storeId }] = queryKey as [string, { storeId: number }];
      if (!storeId) return [];
      const params = new URLSearchParams();
  ...
  // Get sales data
  const { data: salesData = [] } = useQuery<any[]>({
    queryKey: ["/api/admin/sales-analytics", timePeriod, storeId],
    enabled: !!timePeriod && !!storeId,
  });

  // Get category data
  const { data: categoryData = [] } = useQuery<any[]>({
    queryKey: ["/api/admin/category-analytics", storeId],
    enabled: !!storeId,
  });
  ...
  const getOrderPrescription = (order: any) => {
    if (order.prescription) return order.prescription;
    if (order.prescriptionId) {
  ...
  if (statsLoading || storesLoading || medicinesLoading) {
    return (
      <div className="container mx-auto px-4 py-6">
        <div className="space-y-6">
          {[...Array(4)].map((_, i) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Card className="border-blue-200 bg-blue-50">
              ...
              <CardContent>
                ...
                ) : stores.length > 0 && stores[0]?.qrCodeUrl ? (
                  <div className="flex flex-col items-center">
                    <img
                      src={stores[0].qrCodeUrl}
                      alt="Store QR Code"
